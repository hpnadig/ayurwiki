---
title: "Vyāghrī Harītakī"
categories:
  - "Therapeutic use of Formula - Cough"
  - "Therapeutic_use_of_Formula_-_Sinusitis"
  - "Therapeutic_use_of_Formula_-_Tuberculosis"
  - "Therapeutic_use_of_Formula_-_Asthma"
  - "Ayurvedic Medicine"
  - "Therapeutic use of Formula - Sinusitis"
  - "Formulations"
  - "Therapeutic use of Formula - Aphasia"
  - "Therapeutic_use_of_Formula_-_Coryza"
  - "Therapeutic use of Formula - Chronic rhinitis"
  - "Therapeutic_use_of_Formula_-_Chronic_rhinitis"
  - "Therapeutic_use_of_Formula_-_Cough"
  - "Therapeutic use of Formula - Tuberculosis"
  - "Therapeutic_use_of_Formula_-_Aphasia"
  - "Ayurvedic_Medicine"
  - "Therapeutic use of Formula - Asthma"
  - "Therapeutic use of Formula - Coryza"
date: 2018-09-14
---
# Vyāghrī Harītakī

[TOC]

**Vyāghrī Harītakī** is a semisolid preparation made with the ingredients given in the Formulation composition.

## Formulation composition
| Kaṇṭakārī API | [Solanum surattense](Solanum_surattense.md) | Pl. | 4.8 kg |
| --- | --- | --- | --- |
| Jala API for decoction | Water |  | 12.9 l |
| reduced to |  |  | 3.07 l |
| Harītakī API | [Terminalia chebula](Terminalia_chebula.md) | P. | (100 in No.) 1.2 kg |
| Guḍa API | Jaggery |  | 4.8 kg |
| Śuṇṭhī API | [Zingiber officinale](../herbs/Zingiber_officinale_-_Ginger,_Alla,_Adrak,_Ingee,_Allam,_Inchi,_Alha.md) | Rz. | 96 g |
| Marica API | [Piper nigrum](Piper_nigrum.md) | Fr. | 96 g |
| Pippalī API | [Piper longum](Piper_longum.md) | Fr. | 96 g |
| Tvak API | [Cinnamomum zeylanicum](Cinnamomum_zeylanicum.md) | St. Bk. | 48 g |
| Patra (Tvakpatra API) | [Cinnamomum tamala](Cinnamomum_tamala.md) | Lf. | 48 g |
| Elā (Sūkṣmailā API) | [Elettaria cardamomum](Elettaria_cardamomum.md) | Sd. | 48 g |
| Nāgakeśara API | [Mesua ferrea](Mesua_ferrea.md) | Stmn. | 48 g |
| Puṣparasa (Madhu API) | Honey |  | 288 g |

## Method of preparation
* Take raw material of Pharmacopoeial quality.
* Wash, dry and grind ingredient number 1 ([Kvātha Dravya](Kvātha_Dravya.md)) of the formulation composition and pass through sieve number 44 to obtain a coarse powder.
* Clean, dry and powder the ingredients number 5 to 11([Praksepa Dravya](Praksepa_Dravya.md)) of the formulation
* composition and pass through sieve number 85 to obtain a fine powder.

## Description
* A blackish brown, semisolid sticky paste with bitter and astringent taste and spicy pleasant odour.

## Storage
* Store in a cool place in tightly closed containers, protected from light and moisture.

## Therapeutic uses
* Kāsa (Cough), Pratiśyāya (Coryza), Śvāsa (Asthma), Svaraksaya (Aphasia), Pīnasa (Chronic rhinitis / Sinusitis), Rājayakshmā (Tuberculosis).

## Dose
* 5 to 15 g.

## Anupāna
* Water, Milk

## Physico-chemical parameters
| Loss on drying | Not more than 23.0 per cent |
| --- | --- |
| Total ash | Not more than 4.0 per cent |
| Acid-insoluble ash | Not more than 0.15 per cent |
| Alcohol-soluble extractive | Not less than 20.0 per cent |
| Water-soluble extractive | Not less than 68.7 per cent |
| Sulphated Ash | Not more than 0.41 per cent |

## References

## References

1. THE AYURVEDIC PHARMACOPOEIA OF INDIA, PART-II, VOLUME-1, page no 36.

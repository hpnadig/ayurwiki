---
title: "Gokṣurādi Guggulu"
categories:
  - "Ayurvedic Medicine"
  - "Therapeutic use of Formula - Increased frequency and turbidity of urine"
  - "Therapeutic_use_of_Formula_-_Excessive_vaginal_discharge"
  - "Therapeutic use of Formula - Gout"
  - "Therapeutic use of Formula - Diseases due to vata dosha"
  - "Therapeutic use of Formula - Vitiation of semen"
  - "Therapeutic_use_of_Formula_-_Vitiation_of_semen"
  - "Therapeutic use of Formula - Dysuria"
  - "Therapeutic use of Formula - Urinary obstruction"
  - "Therapeutic_use_of_Formula_-_Gout"
  - "Therapeutic use of Formula - Excessive vaginal discharge"
  - "Therapeutic_use_of_Formula_-_Diseases_due_to_vata_dosha"
  - "Formulations"
  - "Therapeutic_use_of_Formula_-_Calculus"
  - "Therapeutic use of Formula - Calculus"
  - "Therapeutic_use_of_Formula_-_Dysuria"
  - "Therapeutic_use_of_Formula_-_Increased_frequency_and_turbidity_of_urine"
  - "Therapeutic_use_of_Formula_-_Urinary_obstruction"
  - "Ayurvedic_Medicine"
date: 2018-09-21
---
# Gokṣurādi Guggulu

[TOC]

**Gokṣurādi Guggulu** Vati is a preparation made with the ingredients in the Formulation composition given below with Guggulu as the basic ingredient.

## Formulation composition
| Gokṣura API | [Tribulus terrestris](Tribulus_terrestris.md) | Fr. | 1.344 kg |
| --- | --- | --- | --- |
| Jala for decoction | Water |  | 8.064 l |
| reduced to API |  |  | 4.032 l |
| Guggulu API | [Commiphora wighti](Commiphora_wighti.md) | O.R. | 336 g |
| Śuṇṭhī API | [Zingiber officinale](../herbs/Zingiber_officinale_-_Ginger,_Alla,_Adrak,_Ingee,_Allam,_Inchi,_Alha.md) | Rz. | 48 g |
| Marica API | [Piper nigrum](Piper_nigrum.md) | Fr | 48 g |
| Pippalī API | [Piper longum](Piper_longum.md) | Fr. | 48 g |
| Harītakī API | [Terminalia chebula](Terminalia_chebula.md) | P | 48 g |
| Bibhītaka API | [Terminalia belerica](Terminalia_belerica.md) | P. | 48 g |
| Āmalakī API | [Emblica officinalis](Emblica_officinalis.md) | P. | 48 g |
| Mustā API | [Cyperus rotundus](Cyperus_rotundus.md) | Rz. | 48 g |

## Method of preparation
* Take all the ingredients of the pharmacopoeial quality.
* Wash, dry and powder the ingredients number 4 to 10 of the formulation composition separately and pass through sieve number 85, weigh them separately in the required quantities and mix.
* Crush weighed quantity of Guggulu Śuddha.
* Wash, dry and powder the Gokṣura and pass through sieve number 40. Soak the coarse powder of
* Gokṣura in 8 times of potable water for 12 h. Gently heat the mixture to boil and continue the boiling to reduce the volume of the mixture to half of its original volume.
* Stop the boiling and filter while still warm through a muslin cloth

## Description
* Spherical pills, black in colour with pleasant odour and bitter taste.

## Storage
* Store in a cool place in tightly closed container, protect from light and moisture.

## Therapeutic uses
Prameha ([increased frequency and turbidity of urine](:Category:Increased_frequency_and_turbidity_of_urine.md)), Mūtrakṛcchra ([dysuria](:Category:Dysuria.md)), Mūtrāghata ([urinary obstruction](:Category:Urinary_obstruction.md)), Aśmāri ([calculus](:Category:Calculus.md)), Pradara ([excessive vaginal discharge](:Category:Excessive_vaginal_discharge.md)), Vātarakta ([gout](:Category:Gout.md)), Vātaroga ([disease due to Vata dosha](:Category:Disease_due_to_vata_dosha.md)), Śukra Doṣa ([vitiation of semen](:Category:Vitiation_of_semen.md)).

## Dose
* 2 - 3 g daily in divided doses

## Anupāna
* Mustā Kvātha, Pāṣāṇabheda Kvātha, Uśīra Kvātha

## Physico-chemical parameters
| Loss on drying | Not more than 15 per cent |
| --- | --- |
| Total ash | Not more than 5 per cent |
| Acid-insoluble ash | Not more than 1 per cent |
| Alcohol-soluble extractive | Not less than 22 per cent |
| Water-soluble extractive | Not less than 29 per cent |
| pH (1% aqueous solution) | 4.42 to 4.79 |

## References

## References

1. THE AYURVEDIC PHARMACOPOEIA OF INDIA, PART-II, VOLUME-II, page no 157.

---
title: "Pūga Khaṇḍa"
categories:
  - "Therapeutic use of Formula - Infertility"
  - "Therapeutic use of Formula - Bleeding piles"
  - "Therapeutic_use_of_Formula_-_Acidity"
  - "Ayurvedic Medicine"
  - "Therapeutic use of Formula - Improves Immunity"
  - "Therapeutic use of Formula - Syncope"
  - "Therapeutic_use_of_Formula_-_Excessive_vaginal_discharge"
  - "Therapeutic use of Formula - Loss of appetite"
  - "Therapeutic use of Formula - Pain"
  - "Therapeutic use of Formula - Improves strength"
  - "Therapeutic use of Formula - Constipation"
  - "Therapeutic_use_of_Formula_-_Tuberculosis"
  - "Therapeutic use of Formula - Dyspepsia"
  - "Therapeutic_use_of_Formula_-_Pain"
  - "Therapeutic use of Formula - Oligospermia"
  - "Therapeutic use of Formula - Anaemia"
  - "Therapeutic use of Formula - Obstruction in urinary tract"
  - "Therapeutic use of Formula - Emesis"
  - "Therapeutic_use_of_Formula_-_Improves_Immunity"
  - "Therapeutic_use_of_Formula_-_Improves_complexion"
  - "Therapeutic_use_of_Formula_-_Bleeding_piles"
  - "Therapeutic use of Formula - Excessive vaginal discharge"
  - "Therapeutic use of Formula - Weakness"
  - "Therapeutic_use_of_Formula_-_Senility"
  - "Formulations"
  - "Therapeutic_use_of_Formula_-_Dyspepsia"
  - "Therapeutic_use_of_Formula_-_Oligospermia"
  - "Therapeutic_use_of_Formula_-_Constipation"
  - "Therapeutic use of Formula - Foetal anomaly"
  - "Therapeutic_use_of_Formula_-_Improves_strength"
  - "Therapeutic use of Formula - Tuberculosis"
  - "Therapeutic use of Formula - Acidity"
  - "Therapeutic_use_of_Formula_-_Obstruction_in_urinary_tract"
  - "Therapeutic use of Formula - Improves vision"
  - "Therapeutic_use_of_Formula_-_Anaemia"
  - "Therapeutic_use_of_Formula_-_Weakness"
  - "Therapeutic_use_of_Formula_-_Emesis"
  - "Therapeutic use of Formula - Thirst"
  - "Therapeutic_use_of_Formula_-_Thirst"
  - "Therapeutic_use_of_Formula_-_Foetal_anomaly"
  - "Therapeutic_use_of_Formula_-_Infertility"
  - "Therapeutic_use_of_Formula_-_Syncope"
  - "Therapeutic_use_of_Formula_-_Improves_vision"
  - "Ayurvedic_Medicine"
  - "Therapeutic use of Formula - Improves complexion"
  - "Therapeutic_use_of_Formula_-_Loss_of_appetite"
  - "Therapeutic use of Formula - Senility"
date: 2018-09-14
---
# Pūga Khaṇḍa

[TOC]

**Pūga Khaṇḍa** is a granular preparation made with the ingredients in the Formulation composition given below.

## Formulation composition
| Pūgaphala API | [Areca catechu](Areca_catechu.md) | Sd. | 384 g |
| --- | --- | --- | --- |
| Sarpi (Go ghṛta API) | Clarified butter from cow’s milk |  | 192 g |
| Varī rasa (Śatāvarī API) | [Asparagus racemosus](Asparagus_racemosus.md) | Rt. | 384 ml |
| Dhātrī rasa (Āmalakī API) | [Phyllanthus emblica(Emblica officinalis)](Phyllanthus_emblica(Emblica_officinalis).md) | Fr. | 384 ml |
| Payasa (Godugdha API) | Cow’s milk |  | 1.5 l |
| Sitā API | Sugar candy |  | 2400 g |
| Hema (Nāgakeśara API) | [Mesua ferrea](Mesua_ferrea.md) | Stmn. | 24 g |
| Ambhodhara (Mustā API) | [Cyperus rotundus](Cyperus_rotundus.md) | Rt. Tr. | 24 g |
| Candana (Śveta candana API) | [Santalum album](Santalum_album.md) | Ht. Wd. | 24 g |
| Śuṇṭhī API | [Zingiber officinale](../herbs/Zingiber_officinale_-_Ginger,_Alla,_Adrak,_Ingee,_Allam,_Inchi,_Alha.md) | Rz. | 24 g |
| Marica API | [Piper nigrum](Piper_nigrum.md) | Fr. | 24 g |
| Pippalī API | [Piper longum](Piper_longum.md) |Fr. | 24 g |  |
| Dhātrī asthimajjā (Āmalakī API) | [Phyllanthus emblica](Phyllanthus_emblica.md) (Emblica officinalis) | Enm. | 24 g |
| Priyālāsthi majjā (Priyala API) | Buchanania lanzan | Enm. | 24 g |
| Tvak API | [Cinnamomum zeylanicum](Cinnamomum_zeylanicum.md) | St. Bk. | 24 g |
| Elā (Sūkṣmailā API) | [Elettaria cardamomum](Elettaria_cardamomum.md) | Sd. | 24 g |
| Patra (Tejapatra API) | [Cinnamomum tamala](Cinnamomum_tamala.md) | Lf. | 24 g |
| Śveta jīraka API | [Cuminum cyminum](Cuminum_cyminum.md) | Fr. | 24 g |
| Kṛṣṇajīraka API | [Carum carvi](Carum_carvi.md) | Fr. | 24 g |
| Śṛṅgātaka API | Trapa natans var. bispinosa | Enm. | 24 g |
| Vaṁśajā (Vaṁśa API) | [Bambusa bambos](../herbs/Bambusa_bambos_-_Indian_Thorny_Bamboo,_ಬಿದಿರು,_Baans,_Kulaymunkil,_Bonguveduru,_Mula,_Baamboo.md) | S.C. | 24 g |
| Jātīphala API | [Myristica fragrans](Myristica_fragrans.md) | Sd. | 24 g |
| Jātīkoṣā (Jātīphala API) | [Myristica fragrans](Myristica_fragrans.md) | Ar. | 24 g |
| Lavaṅga API | [Syzygium aromaticum](Syzygium_aromaticum.md) | Fl. Bd. | 24 g |
| Dhānyaka API | [Coriandrum sativum](Coriandrum_sativum.md) | Fr. | 24 g |
| Kakkola (Kaṅkola API) | [Piper cubeba](Piper_cubeba.md) | Fr. | 24 g |
| Nākulī (Īśvarī API) | [Aristolochia indica](Aristolochia_indica.md) | Rt. | 24 g |
| Tagara API | [Valeriana wallichi](Valeriana_wallichi.md) | Rz. | 24 g |
| Ambu (Hrīvera API) | [Coleus vettiveroides](../herbs/Coleus_vettiveroides.md) | Rt. | 24 g |
| Vīraṇaśiphā (Uśīra API) | [Vetiveria zizanioides](Vetiveria_zizanioides.md) | Rt. | 24 g |
| Bhṛṅga (Bhṛṅgarāja API) | [Eclipta alba](Eclipta_alba.md) | Pl. | 24 g |
| Aśvagandhā API | [Withania somnifera](Withania_somnifera.md) | Rt. | 24 g |

## Method of preparation
* Take all ingredients of pharmacopoeial quality.
* Weigh the ingredients of [Praksepa dravya](Praksepa_dravya.md) numbered 7 to 32 of the Formulation composition, clean dry, powder separately and pass through sieve number 85.
* Take fully mature and dry pūgaphala (areca nuts) and break it into small pieces of about 0.5 –1.0 cm in diameter, tie them in a muslin cloth to form a bundle (Pottali) and immerse into milk in a stainless steel vessel (Dolāyantra vidhi) and boil for 3 h.
* Wash the bundle with warm water (500 to 550) and repeat washing for three times*. Dry these processed Pūgaphala in a tray-dryer at a temperature not exceeding 60degcel.
* Grind the dried pieces and sieve through 85 mesh. Fry the powder in Ghṛta at low temperature between 600-700
* Crush the fresh [Āmalakī](Āmalakī.md), strain through a muslin cloth to obtain juice.

## Description
* Light brown granules with pleasant odour and spicy, sweet, acrid and astringent taste.

## Storage
* Store in a cool place in tightly closed containers, protected from light and moisture.

## Therapeutic uses
* Chardi (Emesis), Śūla (Pain), amlapitta (Acidity), Mūrcchā (Syncope), Vandhyāroga (Infertility), Pradara (Excessive vaginal discharge), Pāndu (Anaemia), Raktārśa (Bleeding piles), Garbhadosha (Foetal anomaly), Jarā (Senility), Śukrakshaya (Oligospermia), Agnimindya (Loss of appetite), Trsht (Thirst), Daurbalya (Weakness), Ajīrṇa (Dyspepsia), Visarga (Constipation), Mūtrasarga (Obstruction in urinary tract), Yakshmā (Tuberculosis), Balya (Improves strength / Immunity), Varna (Improves complexion) and Drshti (Vision).

## Dose
* 12 g daily in divided doses.

## Anupāna
* Essentially to be taken with Milk.

## Physico-chemical parameters
| Loss on drying | Not more than 5 per cent |
| --- | --- |
| Total ash | Not more than 2.40 per cent |
| Acid-insoluble ash | Not more than 1.00 per cent |
| Alcohol-soluble extractive | Not less than 17.0 per cent |
| Water-soluble extractive | Not less than 69.0 per cent |
| pH (1% aqueous solution) | 5.0 to 5.5 |

## References

## References

1. THE AYURVEDIC PHARMACOPOEIA OF INDIA, PART-II, VOLUME-1, page no 28 .

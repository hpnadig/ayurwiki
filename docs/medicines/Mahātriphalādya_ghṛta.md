---
title: "Mahātriphalādya ghṛta"
categories:
  - "Therapeutic use of Formula - Mole"
  - "Ayurvedic Medicine"
  - "Therapeutic_use_of_Formula_-_Mole"
  - "Therapeutic_use_of_Formula_-_Disease_of_eyes"
  - "Therapeutic use of Formula - Itching in eyes"
  - "Therapeutic_use_of_Formula_-_Growth_in_the_layers_of_eyes"
  - "Therapeutic_use_of_Formula_-_Conjunctivitis"
  - "Therapeutic use of Formula - Hypermetropia"
  - "Therapeutic_use_of_Formula_-_Glaucoma"
  - "Therapeutic use of Formula - Cataract"
  - "Therapeutic use of Formula - Conjunctivitis"
  - "Therapeutic_use_of_Formula_-_Chronic_dacrocystitis"
  - "Therapeutic_use_of_Formula_-_Trichiasis"
  - "Therapeutic use of Formula - Growth in the layers of eyes"
  - "Therapeutic use of Formula - Myopia"
  - "Therapeutic use of Formula - Disease of eyes"
  - "Formulations"
  - "Therapeutic_use_of_Formula_-_Blindness"
  - "Therapeutic_use_of_Formula_-_Night_blindness"
  - "Therapeutic use of Formula - Chronic dacrocystitis"
  - "Therapeutic_use_of_Formula_-_Itching_in_eyes"
  - "Therapeutic_use_of_Formula_-_Hypermetropia"
  - "Therapeutic use of Formula - Glaucoma"
  - "Therapeutic use of Formula - Blindness"
  - "Therapeutic_use_of_Formula_-_Cataract"
  - "Therapeutic_use_of_Formula_-_Myopia"
  - "Therapeutic use of Formula - Night blindness"
  - "Ayurvedic_Medicine"
  - "Therapeutic use of Formula - Trichiasis"
date: 2018-09-21
---
# Mahātriphalādya ghṛta

[TOC]

**Mahā Triphalādya ghṛta** is a medicated semisolid preparation made with the ingredients in the Formulation composition given below, with Ghṛta as the basic ingredient.

## Formulation composition
| Triphalā Rasa (Triphalā API) | [Terminalia belerica](Terminalia_belerica.md) | P. | 768 ml |
| --- | --- | --- | --- |
| Kvātha | [Terminalia chebula](Terminalia_chebula.md) | P. |  |
| NIl | [Emblica officinalis](Emblica_officinalis.md) | P. |  |
| Bhṛṅga Rasa (Bhṛṅgarāja API) | [Eclipta alba](Eclipta_alba.md) | Pl. | 768 ml |
| Vṛṣa Rasa (Vāsā API) | [Adhatoda vasica](Adhatoda_vasica.md) | Lf. | 768 ml |
| Śatāvarī Rasa (Śatāvarī API) | [Asparagus racemosus](Asparagus_racemosus.md) | Rt. Tr. | 768 ml |
| Ajā kṣīra API | Goat Milk |  | 768 ml |
| Guḍūcī Rasa (Guḍūcī API) | [Tinospora cordifolia](Tinospora_cordifolia.md) | St. | 768 ml |
| Āmalakī Rasa (Āmalakī API) | [Emblica officinalis](Emblica_officinalis.md) | P. | 768 ml |
| Kaṇā (Pippalī API) | [Piper longum](Piper_longum.md) | Fr. | 8.72 g |
| Sitā API | Sugar candy |  | 8.72 g |
| Drākṣā API | [Vitis vinifera](Vitis_vinifera.md) | Dr. Fr. | 8.72 g |
| Harītakī API | [Terminalia chebula](Terminalia_chebula.md) | P. | 8.72 g |
| Bibhītaka API | [Terminalia belerica](Terminalia_belerica.md) | P. | 8.72 g |
| Āmalakī API | [Emblica officinalis](Emblica_officinalis.md) | P. | 8.72 g |
| Nīlotpala (Utpala API) | [Nymphaea stellata](../herbs/Nymphaea_stellata.md) | Fl | . 8.72 g |
| Madhuka (Yaṣṭī API) | [Glycyrrhiza glabra](Glycyrrhiza_glabra.md) | Rt. | 8.72 g |
| Kṣīra kākolī API | [Fritillaria roylei](../herbs/Fritillaria_roylei.md) | Rt. | 8.72 g |
| Mudhuparṇī (Guḍūcī API) | [Tinospora cordifolia](Tinospora_cordifolia.md) | St. | 8.72 g |
| Nidigdhikā (Kaṇṭakārī API) | [Solanum xanthocarpum](../herbs/Solanum_xanthocarpum_-_Thorny_nightshade,_Kantkari.md) | Pl. | 8.72 g |
| Ghṛta (Goghṛta API) | Clarified butter from Cow  milk |  | 768 g |

## Method of preparation
* Take all ingredients of pharmacopoeial quality.
* Take fresh [Bhṛṅgarāja](Bhṛṅgarāja.md), [Śatāvarī](Śatāvarī.md), [Guḍūcī](Guḍūcī.md) and [Āmalakī](Āmalakī.md) and wash thoroughly with water. Grind and filter with muslin cloth to obtain [Svarasa](../Svarasa.md).
* Take fresh Vāṣā leaves and obtain juice by Puṭapāka method
* Soak the coarse Tripha powder in potable water in the specified ratio for overnight, boil it till the volume is reduced to one fourth of its original volume, cool the *[Kvātha](../Kvātha.md) and filtered through muslin cloth.
* Treat Ghṛta to prepare Murcchita Ghrta.

## Description
* A low melting Gh¤ta, greenish cream in colour, unctuous to touch, no specific odour and taste bitter.

## Storage
* Store in a cool place in tightly closed container, protect from light and moisture.

## Therapeutic uses
* Naktāndhya (Night blindness), Timira (Cataract), Kāca (Cataract), Nīlikā (Mole), Paṭala Arbuda (Growth in the layers of eyes), Netrābhiṣyanda (Conjunctivitis), Adhimantha (Glaucoma), Pakṣmakopa (Trichiasis/entropion), Netraroga (Disease of eyes), Adṛṣṭi (Blindness); Mandadṛṣṭi (Diminished vision); Netrasrāva (Chronic dacrocystitis/epiphora); Netrakaṇḍū (Itching in eyes); Dūradṛṣṭi (Hypermetropia), Samīpadṛṣṭi (Myopia).

## Dose
* 6 to 12 gm twice a day.

## Anupāna
* Warm milk, warm water.

## Physico-chemical parameters
| Refractive index at 40 | 1.4531 to 1.4534 |
| --- | --- |
| Acid value | Not more than 2.9 |
| Saponification value | 0.2100 to 0.2147 |
| Peroxide value | Not more than 15.8 |

## References

## References

1. THE AYURVEDIC PHARMACOPOEIA OF INDIA, PART-II, VOLUME-II, page no 144.
